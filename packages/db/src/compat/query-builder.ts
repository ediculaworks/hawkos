/**
 * QueryBuilder — Supabase-compatible query builder for PostgreSQL
 *
 * Contains all types, SQL helpers, and the QueryBuilder class with
 * select/where/order/limit/join/embed logic.
 */
...
export type FilterOp = '=' | '!=' | '>' | '>=' | '<' | '<=' | 'like' | 'ilike' | 'is' | 'in';
...
export interface ForeignKeyEmbed {
  alias: string | null;
  relation: string;
...
// ── Parse select string ────────────────────────────────────────────────────
export function parseSelectColumns(selectStr: string): {
  columns: string[];
  embeds: ForeignKeyEmbed[];
} {
...
export function parseOrConditions(raw: string): string {
